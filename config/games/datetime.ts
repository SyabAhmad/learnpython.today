      ...
      new CodeBlock(
        [
          ...
          new CodeLine(
            "import clock",
            StateEnum.WRONG,
            "There is no 'clock' module in the standard library.",
          ),
          new CodeLine(
            "import time_module",
            StateEnum.WRONG,
          ...
          new CodeLine(
            ...
            StateEnum.WRONG,
            "The module is 'datetime', and it contains a 'date' class.",
          ),
        ],
        "python",
      ),
      ...
      new CodeBlock(
        [
          ...
          new CodeLine(
            "s = now.format('%Y-%m-%d')",
            StateEnum.WRONG,
            "Date objects don't have a '.format()' method like strings.",
          ),
          new CodeLine(
            "s = now.isoformat()",
            StateEnum.WRONG,
            "isoformat() doesn't take a format string; it returns a fixed ISO 8601 string.",
          ),
          new CodeLine(
            "s = str(now, '%Y-%m-%d')",
            StateEnum.WRONG,
            "The str() function for dates doesn't accept a format string second argument.",
          ),
        ],
        "python",
      ),
      ...
      new CodeBlock(
        [
          ...
          new CodeLine(
            ...
            StateEnum.WRONG,
            "Works for ISO format but doesn't take a custom format string.",
          ),
          new CodeLine(
            "dt = datetime.convert(date_str)",
            StateEnum.WRONG,
            "'convert' is not a datetime method.",
          ),
        ],
        "python",
      ),
      ...
      new CodeBlock(
        [
          ...
          new CodeLine(
            "TimeGap",
            StateEnum.WRONG,
            "There is no 'TimeGap' class in the datetime module.",
          ),
          new CodeLine(
            "Duration",
            StateEnum.WRONG,
            "Not a standard Python class name for this.",
          ),
          new CodeLine(
            "int",
            StateEnum.ERROR,
          ...
          new CodeLine(
            "DateDiff",
            StateEnum.WRONG,
            "Common in other languages, but not Python.",
          ),
        ],
        "python",
      ),
      ...
      new CodeBlock(
        [
          ...
          new CodeLine(
            "yr = d['year']",
            StateEnum.WRONG,
            "Date objects are not dictionaries.",
          ),
          new CodeLine(
            "yr = d.year()",
            StateEnum.WRONG,
            "'.year' is an attribute, not a method.",
          ),
          new CodeLine(
            "yr = d.get('year')",
            StateEnum.WRONG,
            "No '.get()' method exists for dates.",
          ),
        ],
        "python",
      ),
