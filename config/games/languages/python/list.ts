    ...
    .setCodeBlock(
      new CodeBlock(
        [
          ...
          new CodeLine("print(squares)", StateEnum.NORMAL, "Print result"),
          new CodeLine(
            "# Now, let's try to create a list of cubes for odd numbers",
            StateEnum.NORMAL,
            "Comment about next task",
          ),
          new CodeLine(
            "cubes = [x**3 for x in numbers if x % 2 = 1]",
            StateEnum.ERROR,
            "Single '=' used for comparison instead of '=='",
          ),
          new CodeLine(
            "cubes = [x**3 for x in numbers if x % 2 == 1",
            StateEnum.WRONG,
          ...
          new CodeLine(
            "cubes = [x**3 if x % 2 == 1 for x in numbers]",
            StateEnum.WRONG,
            "Incorrect order of 'if' and 'for' clauses",
          ),
          new CodeLine(
            "cubes = (x**3 for x in numbers if x % 2 == 1)",
            StateEnum.WRONG,
            "Creates a generator expression, not a list comprehension",
          ),
          new CodeLine("print(cubes)", StateEnum.NORMAL, "Print result"),
        ],
        "python",
      ),
    )
      ...
      new CodeBlock(
        [
          ...
          new CodeLine(
            "def calculate_volume(length, width, height);",
            StateEnum.WRONG,
            "Semicolon used instead of colon",
          ),
          new CodeLine(
            "def calculate_volume(length, width, height):",
            StateEnum.CORRECT,
          ...
          new CodeLine(
            "calculate_volume = lambda length, width, height:",
            StateEnum.WRONG,
            "Incorrect lambda function syntax",
          ),
          new CodeLine(
    ...
    .setCodeBlock(
      new CodeBlock(
        [
          ...
          new CodeLine("print(fruit_prices)", StateEnum.NORMAL, "Print result"),
          new CodeLine(
          ...
          new CodeLine(
            ...
            StateEnum.ERROR,
            "Incorrect syntax for adding a new key-value pair",
          ),
          new CodeLine(
            "fruit_prices('grape') = 0.8",
            StateEnum.WRONG,
          ...
          new CodeLine(
            "fruit_prices['grape'] = 0.8",
            StateEnum.CORRECT,
            "Correct syntax for adding a new key-value pair",
            10,
          ),
          new CodeLine(
            "fruit_prices.append('grape': 0.8)",
            StateEnum.WRONG,
            "Incorrect use of append method (which is for lists, not dictionaries)",
          ),
          new CodeLine(
            "fruit_prices.add('grape', 0.8)",
            StateEnum.WRONG,
            "Incorrect use of non-existent add method",
          ),
          new CodeLine("print(fruit_prices)", StateEnum.NORMAL, "Print result"),
          new CodeLine(
